...
namespace Api.Repository
{
    public class UserRepository(UserDbContext context) : IUserInterface
    {
        ...
        public async Task<User> GetByIdAsync(int id) => await context!.Users.FirstOrDefaultAsync(u => u.Id == id);

        public async Task<bool> UpdateAsync(User user)
        {
            var getUser = await context.Users.FirstOrDefaultAsync(u => u.Id == user.Id);
            if (getUser != null)
            {
